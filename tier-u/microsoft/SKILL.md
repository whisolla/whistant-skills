...
- `office.js` is **fflate-only** for all OOXML ZIP/container work across DOCX, PPTX, and XLSX (no external runtime dependency).
- `local.aurochs.patchWorkbook()` supports cell-level editing by sheet name + col/row using inline strings (no sharedStrings.xml dependency).
- Local Excel editing supports **cell-level value patching** (strings, numbers, empty) and **plain text string replacement** in OOXML XML parts.
