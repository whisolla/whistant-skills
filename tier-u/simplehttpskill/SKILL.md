...
Make HTTP requests using the native `fetch()` API. Supports all standard
...
None — uses only `fetch()` (built into Whistant's JavaScriptCore runtime).
...
- **body** (string | object, optional): Request body. Objects are auto-serialized to JSON.
- **maxRetries** (number, optional): Retry attempts for transient failures. Default `3`.

## Step-by-Step Workflow

1. Load the client from the skill's scripts directory:

```js
var { SimpleHttpClient } = require("/skills/simplehttpskill/scripts/simplehttp.js");
```
...
```js
// GET
var resp = await client.get("https://api.example.com/items");

// POST with JSON body
var resp = await client.post("https://api.example.com/items", {
  body: { name: "widget" },
});

// PUT with custom headers
var resp = await client.put("https://api.example.com/items/1", {
  headers: { "X-Request-Id": "abc123" },
  body: { name: "updated" },
});

// DELETE
var resp = await client.delete("https://api.example.com/items/1");

// Generic form — any method
var resp = await client.request("PATCH", "https://api.example.com/items/1", {
  body: { qty: 5 },
});
```
