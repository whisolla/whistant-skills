...
function calDavClient(params) {
  ...
  function parseDisplaynames(xml) {
    var results = [];
    var re = /<D:displayname>([^<]+)<\/D:displayname>/g;
    var m;
  ...
  function parseEvents(xml) {
    var events = [];
    var re = /<C:calendar-data[^>]*>([\s\S]*?)<\/C:calendar-data>/gi;
    var m;
  ...
  var api = {
    ...
    listCalendars: async function(calHomeUrl) {
      var res = await fetch(calHomeUrl, {
        method: 'PROPFIND',
      ...
      for (var i = 0; i < hrefs.length; i++) {
        result.push({ href: hrefs[i], displayName: names[i] || hrefs[i] });
      }
      return result;
    },
    ...
    deleteEvent: async function(calUrl, uid, etag) {
      var res = await fetch(calUrl + (calUrl.endsWith('/') ? '' : '/') + uid + '.ics', {
        method: 'DELETE',
        headers: merge(headers, { 'If-Match': etag || '*' }),
        timeout: 15,
      });
