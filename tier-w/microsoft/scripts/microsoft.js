...
async function runFromParams(inputParams) {
  ...
  if (!action) {
    ...
    console.log('  mail inbox --max=N');
    console.log('  mail read <id>');
    console.log('  calendar events --days=N --max=N');
    console.log('  onedrive list --max=N');
    console.log('  onedrive search <query> --max=N');
  ...
  try {
    if (action === 'auth') {
      ...
      if (subaction === 'search') {
        var files = await onedrive.search(args[0] || '', { max: max });
        console.log('files:', files.length);
        files.forEach(function(f) { console.log(f.name + ' (' + f.id + ')'); });
      } else {
        var items = await onedrive.list();
        console.log('items:', items.length);
        items.slice(0, max).forEach(function(f) { console.log(f.name + ' (' + (f.folder ? 'folder' : 'file') + ')'); });
      }
      ...
      console.log('Actions: auth, mail, calendar, onedrive, contacts');
    }
  } catch (e) {
...
const microsoft = { auth, mail, calendar, onedrive, contacts, handler, runFromParams, parseCommand, tokenize };
